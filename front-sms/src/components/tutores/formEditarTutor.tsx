...
interface FormEditarTutorProps {
  tutor: Tutor;
}

const FormEditarTutor = ({ tutor }: FormEditarTutorProps) => {
  ...
  const handleBuscarAlumno = async () => {
    ...
    try {
      ...
      const yaSeleccionado = alumnosSeleccionados.some((a) => a.codigo === alumno.codigo);
      ...
      form.setValue("alumnoIds", nuevosAlumnos.map((a) => a.id).filter((id) => id > 0), { shouldValidate: true });
      setCodigoBusqueda("");
    } catch {
  ...
  const handleRemoveAlumno = (codigo: string) => {
    const nuevosAlumnos = alumnosSeleccionados.filter((a) => a.codigo !== codigo);
    setAlumnosSeleccionados(nuevosAlumnos);
    form.setValue("alumnoIds", nuevosAlumnos.map((a) => a.id).filter((id) => id > 0), { shouldValidate: true });
  };
  ...
  const handleSubmit = async (data: z.infer<typeof formTutorSchema>) => {
    ...
    try {
      ...
      const errorMessage =
        ...
        "Error inesperado al actualizar el tutor.";
      ...
      <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-6">
          ...
          <div className="flex flex-wrap gap-2">
            {alumnosSeleccionados.map((alumno) => (
              ...
              >
                ...
                <button
                  type="button"
                  onClick={() => handleRemoveAlumno(alumno.codigo)}
        ...
        <div className="flex gap-4">
          <Button type="submit" disabled={isLoading} className="cursor-pointer">
            {isLoading ? "Guardando..." : "Actualizar Tutor"}
          </Button>
          <Button
            variant="outline"
            type="button"
...
export default FormEditarTutor
